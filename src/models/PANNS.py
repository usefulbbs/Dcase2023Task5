...
class Cnn14(nn.Module):
    def __init__(self, sample_rate=22050, window_size=431, hop_size=256, mel_bins=128, fmin=0, 
        fmax=11025, classes_num=755,layer_nums=6):
    ...
    def extract_feature(self,x,if_pooling=True,step=0):
        _,seq_len,n_mels = x.shape
        x = x.unsqueeze(1)
        x1 = x[:,:,:seq_len//2]
        x2 = x[:,:,seq_len//2:]
        lay_num = min(max(self.layer_nums,1),6) 
        if step==0:  # use x1 as input
            
            if if_pooling: #[B,1,431,128]====>[B,64*2**(lay_num-1),431,128//2**(lay_num)]
               down_sample_rate = 2**lay_num
               pad_ = 2**lay_num - 431 % down_sample_rate if 431 % down_sample_rate!=0 else 0
               x1 = torch.functional.F.pad(x1,[0,0,0,pad_])
               for i in range(lay_num):
                   x1=eval(
                       f"self.conv_block{i+1}(x1,(2,2))"
                       )
               x1 = x1.repeat_interleave(2**lay_num,dim=2)[:,:,:seq_len//2]
            else: #[B,1,431,128]====>[B,64*2**(lay_num-1),431,128]
               for i in range(lay_num):
                    x1=eval(
                       f"self.conv_block{i+1}(x1,(1,1))"
                       )
            return x1
        else: # use x as input
            if if_pooling: #[B,1,431,128]====>[B,64*2**(lay_num-1),431,128//2**(lay_num)]
               down_sample_rate = 2**lay_num
               pad_ = 2**lay_num - 431 % down_sample_rate if 431 % down_sample_rate!=0 else 0
               x = torch.functional.F.pad(x,[0,0,0,pad_])
               for i in range(lay_num):
                   x=eval(
                       f"self.conv_block{i+1}(x,(2,2))"
                       )
               x = x.repeat_interleave(2**lay_num,dim=2)[:,:,:seq_len]
            else: #[B,1,431,128]====>[B,64*2**(lay_num-1),431,128]
               for i in range(lay_num):
                   x=eval(
                       f"self.conv_block{i+1}(x,(1,1))"
                       )
            return x
