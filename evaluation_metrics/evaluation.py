...
def remove_shots_from_ref(ref_df, number_shots=5): # 移除shot部分
    
    ref_pos_indexes = select_events_with_value(ref_df, value=POS_VALUE) # 选出所有pos的段
    ref_n_shot_index = ref_pos_indexes[number_shots-1] # 找到第5段pos
    # remove all events (pos and UNK) that happen before this 5th event
    events_to_drop = ref_df.index[ref_df['Endtime'] <= ref_df.iloc[ref_n_shot_index]['Endtime']].tolist() # 移除时间小于支持集的段
...
def compute_tp_fp_fn(pred_events_df, ref_events_df):
    ...
    # total number of poss events = TP + FN
    # TP-->你预测为P,且预测正确, FP---> 你预测为P，但你预测错了（即实际上为N）
    ...
    ref_pos_indexes = select_events_with_value(ref_events_df, value=POS_VALUE) # 选出target文件中 所有的pos 对应的index

    if "Q" not in pred_events_df.columns:
        pred_events_df["Q"] = POS_VALUE
    pred_pos_indexes = select_events_with_value(pred_events_df, value=POS_VALUE) #选出预测文件中所有Pos对应的index

    ref_1st_round = build_matrix_from_selected_rows(ref_events_df, ref_pos_indexes) # 求得一个2*pos_len的矩阵,记录开始时间和结束时间
    pred_1st_round = build_matrix_from_selected_rows(pred_events_df, pred_pos_indexes)

    m_pos = metrics.match_events(ref_1st_round, pred_1st_round, min_iou=MIN_IOU_TH) # 要求时间尽可能的有重合，计算每段重叠情况，重叠超过0.3即为合格
    ...
    ref_2nd_round = build_matrix_from_selected_rows(ref_events_df, ref_unk_indexes) # 建立unkown的 矩阵

    unmatched_pred_events = list(set(range(pred_1st_round.shape[1])) - set(matched_pred_indexes)) # 计算为预测对的集合
    pred_2nd_round = pred_1st_round[:, unmatched_pred_events] # 找到没有预测对的，剩余矩阵

    m_unk = metrics.match_events(ref_2nd_round, pred_2nd_round, min_iou=MIN_IOU_TH)  # 没有预测对的，当做unk去匹配

    # print("# Positive matches between Ref and Pred :", len(m_pos))
    # print("# matches with Unknown events: ", len(m_unk))
    
    tp = len(m_pos)  # 预测对的，作为TP
    fp = pred_1st_round.shape[1] - tp - len(m_unk) # 即不和pos匹配，也不和unk匹配的，设置为fp
    
    ## compute unmatched pos ref events:
    count_unmached_pos_ref_events = len(ref_pos_indexes) - tp  # 计算 FN ,预测为负，但实际为P
...
def evaluate(pred_file_path, ref_file_path, team_name, dataset, savepath, metadata=[]): # ref_file : 指测试标注文件
    ...
    gt_file_structure = {}
    gt_file_structure[dataset] = {} # dataset-->VAL
    inv_gt_file_structure = {}
    list_of_subsets = os.listdir(ref_file_path) # validata 分成两个文件夹
    ...
    #read prediction csv
    pred_csv = pd.read_csv(pred_file_path, dtype=str) # 读predict 结果
    #verify headers:
    ...
    pred_events_by_audiofile = dict(tuple(pred_csv.groupby('Audiofilename'))) # 根据audio文件名 分组

    counts_per_audiofile = {}
    for audiofilename in list(pred_events_by_audiofile.keys()): # 依次遍历不同组
        ...
        ref_events_this_audiofile = remove_shots_from_ref(ref_events_this_audiofile_all, number_shots=N_SHOTS) # 从标注文件中，移除支持集
    ...
    for audiofilename in list_all_audiofiles:
        if audiofilename not in counts_per_audiofile.keys():  # 根据每个文件的TP,FP计算值
            ...
            total_n_pos_events_in_audiofile =  len(select_events_with_value(ref_events_this_audiofile, value=POS_VALUE)) # 总的POS事件
    ...
    for data_set in list_sets_in_mainset:
        ...
        if metadata:
            ...
            for audiofile in  list_audiofiles_in_set:
                ...
                scores_per_audiofile[audiofile] = compute_scores_from_counts(counts_per_audiofile[audiofile]) # 计算每个文件的presion, f1 score
                tp = tp + counts_per_audiofile[audiofile]["TP"]
                fn = fn + counts_per_audiofile[audiofile]["FN"]
                fp = fp + counts_per_audiofile[audiofile]["FP"]
            ...
            #  compute scores per subset
            scores_per_set[data_set] = compute_scores_from_counts(counts_per_set[data_set]) # 计算一个文件夹下的
...
if __name__ == "__main__":
    ...
    parser.add_argument('-pred_file', default="",type=str, help='csv predictions file')
    parser.add_argument('-ref_files_path',default="", type=str, help='path to the ground truth csvs folder')
    parser.add_argument('-metadata', type=str, help="path for metadata json. Participants may ignore this option.")
    parser.add_argument('-team_name', default="PKU_ADSP",type=str, help='team identification')
    parser.add_argument('-dataset', type=str, default="VAL",help="which set to evaluate: EVAL or VAL")
    parser.add_argument('-savepath', default="../evaluation_metrics/",type=str,help="path where to save the report to")
